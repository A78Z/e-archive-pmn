...
import { Settings, Users, UserPlus, UserCheck, HardDrive, FileText, TrendingUp, Settings2, UserCog, Trash2, Edit, AlertTriangle, User2, UserX, ShieldCheck, Clock, CheckCircle2, XCircle } from 'lucide-react';
...
export default function AdministrationPage() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      const [usersResult, docsResult, requestsResult] = await Promise.all([
        supabase.from('users').select('*').order('created_at', { ascending: false }),
        supabase.from('documents').select('*'),
        supabase.from('access_requests').select(`
          *,
          document:documents(name),
          requester:users!access_requests_requested_by_fkey(full_name, email)
  ...
  const storageUsed = 1.6;
  const totalStorage = 1024;
  const totalDocuments = documents.length;
  const documentsGrowth = 12;
            ...
            <div className="relative flex items-center justify-center">
              <svg className="h-32 w-32 transform -rotate-90">
                ...
                <circle
                  ...
                  strokeWidth="12"
                  fill="none"
                  strokeDasharray={`${(activeUsers / totalUsers) * 351.86} 351.86`}
                  strokeLinecap="round"
                />
              </svg>
