...
export default function SharesPage() {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on('postgres_changes', { event: '*', schema: 'public', table: 'shares' }, () => fetchShares())
      .subscribe();
  ...
  const fetchShares = async () => {
    ...
    try {
      const { data: myShares } = await supabase
        .from('shares')
        .select('*, document:documents(*), shared_with_user:users!shares_shared_with_fkey(*)')
        .eq('shared_by', profile.id)
        .order('created_at', { ascending: false });

      const { data: withMe } = await supabase
        .from('shares')
        .select('*, document:documents(*), shared_by_user:users!shares_shared_by_fkey(*)')
        .eq('shared_with', profile.id)
        .order('created_at', { ascending: false });

      setSharedByMe(myShares || []);
      setSharedWithMe(withMe || []);
      setReceivedCount(withMe?.length || 0);
      setSharedCount(myShares?.length || 0);

      const now = new Date();
      const oneDayAgo = new Date(now.getTime() - 24 * 60 * 60 * 1000);
      const recent = [...(myShares || []), ...(withMe || [])].filter(
        share => new Date(share.created_at) > oneDayAgo
      );
  ...
  const handleCopyLink = (share: ShareWithDetails) => {
    ...
    const link = `${window.location.origin}/api/share/${share.share_token}`;
  ...
  const handleDownload = async (share: ShareWithDetails) => {
    ...
    try {
      const { data, error } = await supabase.storage
        .from('documents')
        .download(share.document.file_path);
    ...
    try {
      ...
      for (const share of sharesToDownload) {
        ...
        try {
          ...
          if (error) throw error;
          zip.file(share.document.name, data);
        } catch (error) {
          console.error(error);
        }
      }
                ...
                {previewShare.share_token && (
                  <div className="space-y-2 col-span-2">
                    ...
                    <div className="flex gap-2">
                      <Input
                        readOnly
                        value={`${window.location.origin}/api/share/${previewShare.share_token}`}
                        className="text-sm"
                      />
