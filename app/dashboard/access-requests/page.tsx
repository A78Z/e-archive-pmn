...
export default function AccessRequestsPage() {
  ...
  const fetchDocuments = async () => {
    const { data } = await supabase
      .from('documents')
      .select('id, name, category, uploaded_by')
      .order('name');
    ...
    const { data: incoming } = await supabase
      ...
      .select(`
        *,
        ...
        reviewer:users!access_requests_reviewed_by_fkey(*)
      `)
      ...
      <AlertDialog open={showRejectDialog} onOpenChange={setShowRejectDialog}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Êtes-vous sûr de vouloir rejeter cette demande ? Cette action est définitive.
            </AlertDialogDescription>
          </AlertDialogHeader>
